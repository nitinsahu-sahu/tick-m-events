...
import {placeABidProjectStatusUpdate } from "src/redux/actions/organizer/pageEvents";
...
export const SignedStatusModal = ({
    open,
    onClose,
    currentStatus,
    contract
    ...
    const handleStatusConfirm = async (newStatus: string) => {
        ...
        try {
            const res = await dispatch(placeABidProjectStatusUpdate(contract?.projectDetails?._id, newStatus, contract?.winningBid?.providerId?._id)) as unknown as StatusUpdateResponse;
            if (res?.status === 200) {
                onClose();
            }
        } catch (error) {
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            ...
            <DialogActions sx={{ px: 3, pb: 2 }}>
                ...
                <Button
                    onClick={() => handleStatusConfirm(selectedStatus)}
                    variant="contained"
                    sx={{
